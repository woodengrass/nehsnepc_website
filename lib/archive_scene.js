...
export async function createArchiveScene(canvas, onExitPageFrame) {
  ...
  // Mobile omits satellite cards, so avoid downloading and decoding their unused images.
  const photoNames = isMobile
    ? [...new Set(['focus', ABOUT_EXIT_CONTENT.photo, ...ABOUT_STATIONS.map((station) => station.photo)])]
    : Object.keys(ABOUT_PHOTOS);
  const photoSources = [...new Set(photoNames.map((name) => ABOUT_PHOTOS[name]))];
  const useAvif = await supportsAvif(getAvifUrl(photoSources[0]));
