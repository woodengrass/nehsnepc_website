...
import { ABOUT_EXIT_CONTENT, ABOUT_PHOTOS, ABOUT_STATIONS, ABOUT_TUNNEL_PHOTOS } from './about_content';
const CAMERA_PROGRESS_STIFFNESS = 90;
const CAMERA_PROGRESS_DAMPING = 19;
const EXIT_PROGRESS_STIFFNESS = 72;
const EXIT_PROGRESS_DAMPING = 17;
...
const STATION_LAYOUTS = [
  {
    ...
    mainHeight: 1.47,
    satellites: [
      { x: -3.15, y: 1.65, z: -0.65, width: 0.58, height: 0.39, rotation: -0.12 },
      { x: 3.15, y: 1.58, z: -0.9, width: 0.58, height: 0.39, rotation: 0.1 },
      { x: -3.08, y: -1.65, z: 0.3, width: 0.42, height: 0.63, rotation: 0.08 },
      { x: 3.14, y: -1.62, z: -0.35, width: 0.42, height: 0.63, rotation: -0.08 }
    ],
    mobile: { textX: 0, textY: 0.92, photoX: 0.1, photoY: -0.94, scale: 0.66 }
  },
  {
    // 第二站為出口前的最後停駐位置，保留完整的轉動進場。
    z: -23.8,
    textX: -1.24,
    photoX: 1.45,
    mainWidth: 1.35,
    mainHeight: 0.9,
    satellites: [
      { x: -2.25, y: 2.15, z: -3.05, width: 0.28, height: 0.47, rotation: -0.12 },
      { x: 2.95, y: 2.4, z: -2.1, width: 0.42, height: 0.63, rotation: 0.1 },
      { x: -2.18, y: -2.15, z: -2.1, width: 0.42, height: 0.63, rotation: 0.08 },
      { x: 2.75, y: -1.9, z: -1.85, width: 0.48, height: 0.6, rotation: -0.08 }
    ],
...
function createExitPage(isMobile, image) {
  ...
  // 3D 牆面使用與 DOM 第三頁相同的紙張、編號與標題，靠近到滿版後才能無縫交接。
  ...
  context.fillText('03', frameX + 34, frameY + frameHeight - 92);
  context.font = '16px Arial, sans-serif';
  context.letterSpacing = '4px';
  context.fillText('NEHS', frameX + 34, frameY + frameHeight - 42);
...
function createTunnelPhotoCard(sourceTexture, width, height) {
  const frameWidth = width + 0.035;
  const frameHeight = height + 0.035;
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = Math.max(1, Math.round(canvas.width * frameHeight / frameWidth));
  const context = canvas.getContext('2d');
  ...
  const imageScale = Math.min(photoWidth / image.width, photoHeight / image.height);
  const drawWidth = image.width * imageScale;
  const drawHeight = image.height * imageScale;
  const drawX = inset + (photoWidth - drawWidth) * 0.5;
  const drawY = inset + (photoHeight - drawHeight) * 0.5;
  context.drawImage(image, drawX, drawY, drawWidth, drawHeight);
...
function createStation(layout, textures, stationIndex, isMobile) {
  ...
  const mainTexture = textures[layout.photo];
  const mainPhoto = createPhotoCard(mainTexture, layout.mainWidth ?? 1.12, layout.mainHeight ?? 0.86);
  ...
  // 手機同樣配置衛星照片，實際位置在下方 mobile layout 另外收斂，避免壓到標題與主照片。
  layout.satellites.forEach((item, index) => {
    const photoTexture = textures[layout.satellitePhotos[index]];
    // 漸進載入第一階段尚未取得衛星紋理時跳過，背景補載完成後由掛載邏輯補上。
    if (!photoTexture) return;
    const photo = createPhotoCard(photoTexture, item.width, item.height);
...
function createTunnelGallery(textures, isMobile) {
  ...
  for (let index = 0; index < photoCount; index += 1) {
    const galleryScale = isMobile ? 0.48 : 0.58;
    ...
    const photo = createTunnelPhotoCard(texture, width, height);
...
const AVIF_CACHE_KEY = 'about-avif-support';
let cachedAvifSupport = null;

function readCachedAvifSupport() {
  if (cachedAvifSupport !== null) return cachedAvifSupport;
  try {
    const stored = window.sessionStorage?.getItem(AVIF_CACHE_KEY);
    if (stored === '1') cachedAvifSupport = true;
    else if (stored === '0') cachedAvifSupport = false;
  } catch {
    // sessionStorage 不可用時忽略快取。
  }
  return cachedAvifSupport;
}

function writeCachedAvifSupport(value) {
  cachedAvifSupport = value;
  try {
    window.sessionStorage?.setItem(AVIF_CACHE_KEY, value ? '1' : '0');
  } catch {
    // sessionStorage 不可用時忽略快取。
  }
}
...
export async function createArchiveScene(canvas, onExitPageFrame) {
  ...
  // Mobile 同樣渲染衛星照片，必須載入其紋理。
  const photoNames = isMobile
    ? [...new Set(['focus', ABOUT_EXIT_CONTENT.photo, ...ABOUT_STATIONS.flatMap((station) => [station.photo, ...station.satellitePhotos])])]
    : Object.keys(ABOUT_PHOTOS);
  const photoSources = [...new Set(photoNames.map((name) => ABOUT_PHOTOS[name]))];
  // AVIF 探測與 renderer 初始化並行：先啟動探測（快取命中直接 resolve），再做同步初始化，最後才 await。
  const cachedAvif = readCachedAvifSupport();
  const avifProbe = cachedAvif !== null
    ? Promise.resolve(cachedAvif)
    : supportsAvif(getAvifUrl(photoSources[0])).then((supported) => {
      writeCachedAvifSupport(supported);
      return supported;
    });
  ...
  const useAvif = await avifProbe;
  async function loadTexture(url) {
    const preferredUrl = useAvif ? getAvifUrl(url) : url;
    try {
      return await textureLoader.loadAsync(preferredUrl);
    } catch (error) {
  ...
  // 第一階段只載入首屏主體（對焦圖、出口圖、各站主圖、隧道圖）；
  // 衛星照片等場景成形後再於背景補上，避免慢網路黑屏等待全部紋理。
  const criticalNames = [...new Set(['focus', ABOUT_EXIT_CONTENT.photo, ...ABOUT_STATIONS.map((station) => station.photo), ...ABOUT_TUNNEL_PHOTOS])];
  const criticalUrls = [...new Set(criticalNames.map((name) => ABOUT_PHOTOS[name]))];
  const deferredUrls = photoSources.filter((url) => !criticalUrls.includes(url));
  const texturesByUrl = new Map();
  const criticalList = await Promise.all(criticalUrls.map(loadTexture));
  criticalList.map(trackTexture).forEach((texture, index) => {
    texturesByUrl.set(criticalUrls[index], texture);
  });
  const textures = Object.fromEntries(
    photoNames.map((name) => [name, texturesByUrl.get(ABOUT_PHOTOS[name])])
  );
  ...
  const stations = STATION_LAYOUTS.map((layout, index) => {
    ...
    station.userData.sourceLayout = responsiveLayout;
    if (isMobile) {
      ...
      station.children.slice(2).forEach((photo, photoIndex) => {
        ...
        // 同一 X 位置不再重疊：上下分排並錯開 Z，避免完全重合造成 z-fighting。
        photo.position.set(direction * 1.38, -1.52 - row * 0.72, -0.65 - photoIndex * 0.06);
      });
      station.userData.baseChildren = station.children.map((child) => ({
        object: child,
        x: child.position.x,
  ...
  for (let index = 0; index < fragmentCount; index += 1) {
    ...
    const exitDepth = isExitFragment
      ...
      : 0;
    // 前兩站沿深度切成等量區間，每區只加入少量隨機偏移，避免純隨機造成大片空洞。
  ...
  function resize() {
    ...
    if (portal.visible) {
      ...
      isPortalTextureDirty = false;
    } else {
      isPortalTextureDirty = true;
    }
  ...
  function render(time = performance.now()) {
    ...
    // 出口同樣以物理阻尼追隨捲動目標，保留碎片匯聚與轉彎的收束感；
    // 目標抵達終點時則在接近完成後直接收斂，讓 DOM 最終頁能穩定交接。
    const exitDistance = targetExitProgress - exitProgress;
    const exitAcceleration = exitDistance * EXIT_PROGRESS_STIFFNESS
      - exitProgressVelocity * EXIT_PROGRESS_DAMPING;
    exitProgressVelocity += exitAcceleration * deltaSeconds;
    exitProgress += exitProgressVelocity * deltaSeconds;
    if (
      (Math.abs(exitDistance) < 0.00005 && Math.abs(exitProgressVelocity) < 0.0005)
      || (targetExitProgress >= 0.999 && exitProgress >= 0.992)
    ) {
      exitProgress = targetExitProgress;
      exitProgressVelocity = 0;
    }
    exitProgress = MathUtils.clamp(exitProgress, 0, 1);
    if ((exitProgress === 0 && exitProgressVelocity < 0) || (exitProgress === 1 && exitProgressVelocity > 0)) {
      exitProgressVelocity = 0;
    }
    ...
    if (portal.visible && isPortalTextureDirty) {
      updatePortalTexture(portalTexture, canvas.clientWidth, canvas.clientHeight);
      isPortalTextureDirty = false;
    }
  ...
  function scheduleRender() {
    if (disposed || animationFrame !== null || !isPageVisible || !isSceneInView || isRenderingPaused) return;
    animationFrame = window.requestAnimationFrame(animate);
  }

  function animate(time) {
    animationFrame = null;
    if (disposed || !isPageVisible || !isSceneInView || isRenderingPaused) return;
    render(time);
    scheduleRender();
  }
  ...
  function setExitProgress(nextProgress) {
    targetExitProgress = MathUtils.clamp(nextProgress, 0, 1);
  }

  function setRenderingPaused(shouldPause) {
    if (isRenderingPaused === shouldPause) return;
    isRenderingPaused = shouldPause;
    if (shouldPause && animationFrame !== null) {
      window.cancelAnimationFrame(animationFrame);
      animationFrame = null;
      return;
    }
    if (!shouldPause) resumeRendering();
  }
  ...
  // 第二階段：背景補載衛星紋理並掛載到各站，定稿畫面與一次載完完全一致。
  // 渲染迴圈持續運行，新網格下一幀自動出現；若中途銷毀則直接釋放避免洩漏。
  Promise.all(deferredUrls.map(async (url) => {
    try {
      return { url, texture: trackTexture(await loadTexture(url)) };
    } catch {
      return null;
    }
  })).then((results) => {
    ...
    loaded.forEach(({ url, texture }) => {
      ...
      const name = photoNames.find((candidate) => ABOUT_PHOTOS[candidate] === url);
      if (name) textures[name] = texture;
    });
    stations.forEach((station, stationIndex) => {
      const layout = station.userData.sourceLayout;
      if (!layout || station.userData.satellitesAttached) return;
      station.userData.satellitesAttached = true;
      layout.satellites.forEach((item, index) => {
        const photoTexture = textures[layout.satellitePhotos[index]];
        if (!photoTexture) return;
        const photo = createPhotoCard(photoTexture, item.width, item.height);
